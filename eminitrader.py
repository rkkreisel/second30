...
class EMiniTrader(AppWrapper, AppClient):
    def __init__(self):
...
def main(args):
    ...
    console.info("Started EMiniTrader v{}".format(config.VERSION))

    app = EMiniTrader()

    try:
        console.info("Connecting to TWS API  on HOST {} and PORT {}".format(config.HOST, args.port))
        app.connect(config.HOST, args.port, clientId=0)
        if app.isConnected():
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="EMini Trader")
